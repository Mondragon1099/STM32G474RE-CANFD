 ...
 *
 *   The position controller integrates the commanded rate (same as the
 *   original open-loop-on-encoder design).  What's new: the AS5600 on
 *   ADC5_IN1 (PA8) is read every control tick and its filtered wheel
 *   angle drives stepper_set_target_deg().  Move the wheel, the rudder
 *   tracks.  The TIM3 quadrature encoder on the left motor is shown in
 *   the debug printf as an independent witness, exactly as before.
 *
 *   If the AS5600 read fails AS5600_FAIL_LIMIT times in a row, the
 *   target is FROZEN at its last good value (rudder holds position)
 *   until the sensor recovers - prevents the rudder from snapping to
 *   zero on a bad wire.
 *
...
/* --- AS5600 closed-loop feedback (analog on ADC5_IN1 / PA8) ---
 * AS5600_SIGN        : flip if the magnet rotates opposite to commanded dir.
 *                      Same convention as ENCODER_SIGN - verify on bench.
 * AS5600_FILT_ALPHA  : IIR low-pass coefficient on the raw reading.
 *                      Lower = smoother but laggier; 0.15 is a good start.
 * AS5600_OVERSAMPLE  : ADC samples averaged per as5600_read_deg() call.
 * AS5600_FAIL_LIMIT  : consecutive bad ADC reads before we declare the
 *                      sensor unhealthy and freeze the target. */
 ...
 * STEER_RUDDER_RANGE_DEG : max rudder target in degrees, clamped both sides.
 *                          Wheel positions outside this just saturate.
 * STEER_GAIN             : output_deg per wheel_deg.  1.0 = direct 1:1.
 *                          Use 0.5 for soft steering, 2.0 for sharp, etc.
 * STEER_TARGET_DEADBAND_DEG : if |wheel| is smaller than this, target = 0.
 *                          Absorbs sensor noise around centre. */
...
/* AS5600 rudder-position feedback state.
 *   as5600_deg        : filtered raw reading in [0, 360)
 *   as5600_zero_deg   : boot-captured logical centre, -1 = not captured yet
 *   as5600_ok         : 1 once we have valid filtered data
 *   as5600_fail_count : consecutive failed ADC reads (used for fault gate) */
...
/* ---------------------------------------------------------------------------
 * Wrap-aware angle helpers (used by the AS5600 IIR filter and the zero-diff
 * math).  angle_diff_deg returns the signed shortest path target-current in
 * [-180, +180], so a step from 359 -> 1 reads as +2, not -358.
 * --------------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------------
 * AS5600 analog read on ADC5_IN1 (PA8).
 *
 * The AS5600's OUT pin outputs 0..Vcc proportional to 0..360 deg (default
 * OUTS=00 mode).  With 12-bit ADC and Vcc tied to the STM32's 3V3 rail,
 * angle = (adc * 360) / 4096.
 *
 * Oversamples AS5600_OVERSAMPLE times per call to knock down noise.
 * Returns degrees in [0, 360) on success, -1.0 on ADC error.
 * --------------------------------------------------------------------------- */
static float as5600_read_deg(void)
{
    uint32_t sum = 0;
...
/* ---------------------------------------------------------------------------
 * AS5600 tick - call once per control period (every RATE_STEPPER_MS ms).
 *
 *   1. Sample the sensor.  -1 means an ADC error - count it toward the
 *      health fault gate.
 *   2. On the FIRST valid reading, seed the filter AND capture the boot
 *      zero in one shot.  Doing this only on a real successful read
 *      prevents a failed first ADC from latching the zero to 0.0, which
 *      would peg the measured angle at the wrong reference forever.
 *   3. Otherwise, run a wrap-aware IIR low-pass: smooth in the short way
 *      around the 0/360 seam so the filter doesn't lurch -360 deg when
 *      the magnet crosses zero.
 * --------------------------------------------------------------------------- */
static void as5600_tick(void)
{
...
/* Signed wheel angle from boot-zero, wrap-aware in [-180, +180] deg.
 * Returns 0 before the first valid read so callers don't see garbage
 * during boot. */
float as5600_get_wheel_deg(void)
{
...
/* Recapture the current AS5600 reading as the new logical centre.  Call
 * after mechanically centring the wheel, or wire it to the Zero_Steering
 * pushbutton. */
void as5600_zero_capture(void)
{
...
/* ---------------------------------------------------------------------------
 * Steering tick - call once per control period (every RATE_STEPPER_MS ms).
 *
 *   1. Refresh AS5600 reading (filtered, wrap-aware, fault-tracked).
 *   2. Poll Zero_Steering button - hold while wheel is centred to reset
 *      the logical zero.  Pin is configured PULLDOWN, HIGH = pressed.
 *   3. If sensor is unhealthy, leave the target where it was so the
 *      rudder holds position instead of snapping to centre.
 *   4. Map wheel angle -> rudder target with gain, deadband, clamp.
 *   5. Push the result into the position controller via the public API.
 * --------------------------------------------------------------------------- */
void steering_tick(void)
{
    as5600_tick();
 ...
 *
 * This loop tracks COMMANDED position (integrates the executed rate).
 * The TIM3 encoder readback (encoder_get_deg) is independent and only
 * used for display.  The AS5600 isn't in this path - it drives the
 * setpoint from main() via stepper_set_target_deg().
 * --------------------------------------------------------------------------- */
void stepper_set_target_deg(float output_deg)
{
...
int main(void)
{
  ...
  /* Try to prime the AS5600 so the boot-zero is captured before any
   * target is set.  If the sensor doesn't answer in ~100 ms we keep going
   * anyway - the runtime fault gate in stepper_position_tick() will keep
   * the motors stopped until as5600_ok goes true.  This means UART comes
   * up and prints even when the AS5600 is disconnected, which is what you
   * want during bring-up. */
   ...
   *
   * steering_tick() reads the AS5600 wheel, applies the gain/deadband/
   * clamp from the STEER_* defines, and pushes the result into the
   * position controller via stepper_set_target_deg().
   * stepper_position_tick() then drives the motors toward that target.
   *
   * Both ticks run on the same RATE_STEPPER_MS schedule (2 ms = 500 Hz).
   * --------------------------------------------------------------------- */
  ...
  while (1)
  {
      ...
      /* --- Control tick: wheel -> target -> motors --- */
      if (now - last_tick >= RATE_STEPPER_MS)
      {
          steering_tick();          /* AS5600 wheel -> stepper_set_target_deg */
          stepper_position_tick();  /* P controller drives motors to target  */
          last_tick = now;
      }
      ...
      if (now - last_print >= 100U)
      {
          printf("TGT=%+7.2f  CMD=%+7.2f  ENC=%+7.2f  err=%+7.2f  at_tgt=%u\r\n",
                 ...
                 (double)(stepper_get_target_deg() - stepper_get_output_deg()),
                 stepper_at_target());
          last_print = now;
      }
    /* USER CODE END WHILE */
