 ...
 *
 *   Stepper Left  PUL              PB6  → TIM8  CH1   (AF5_TIM8)
 *   Stepper Left  DIR              PB4  GPIO output
 *   Stepper Left  EN  (active LOW) PB5  GPIO output
 *
 *   Stepper Right PUL              PB15 → TIM15 CH2   (AF1_TIM15 on STM32G4!)
 ...
 *
 * ── About the PB15 / TIM15_CH2 alternate function ─────────────────────────
 *   On STM32G4, PB15 → TIM15_CH2 is **AF1** (not AF14 like on STM32L4).
 *   This file does NOT hard-code the AF in MX_TIM15_Init — it calls
 *   HAL_TIM_MspPostInit(&htim15), which is auto-generated by CubeMX in
 *   stm32g4xx_hal_msp.c from the .ioc and uses GPIO_AF1_TIM15. Same for
 *   TIM8 / PB6 / TIM8_CH1 (AF5_TIM8). Keeping the GPIO/AF setup in msp.c
 *   means future .ioc regenerations stay consistent with main.c.
 *
...
/* USER CODE BEGIN PFP */
/* HAL_TIM_MspPostInit lives in stm32g4xx_hal_msp.c; declare it here so we
 * can call it from MX_TIM8_Init / MX_TIM15_Init to configure PB6 (AF5) and
 * PB15 (AF1 on STM32G4!) as the timer-output alternate functions. */
 ...
 *
 * GPIO + AF + peripheral-clock setup for ADC, FDCAN, I2C, TIM is delegated
 * to the matching HAL_xxx_MspInit() callbacks inside stm32g4xx_hal_msp.c.
 * Those callbacks are invoked automatically by HAL_xxx_Init().
 *
 * For the timer PUL outputs (TIM8_CH1 on PB6, TIM15_CH2 on PB15), the
 * timer-channel pin needs an extra GPIO setup that HAL doesn't do as part
 * of HAL_TIM_PWM_Init — CubeMX generates that into HAL_TIM_MspPostInit().
 * We call HAL_TIM_MspPostInit(&htim8) / (&htim15) explicitly at the end of
 * MX_TIM8_Init / MX_TIM15_Init. Result: PB6 gets AF5_TIM8, PB15 gets
 * AF1_TIM15 (correct values for STM32G4), straight from the .ioc.
 * ========================================================================= */
...
/* TIM2 — free-running 1 µs counter (Hall timing).
 * Clock enable + NVIC happen in HAL_TIM_Base_MspInit() inside msp.c. */
static void MX_TIM2_Init(void)
{
...
/* TIM8 — left motor PWM. PB6 is configured as TIM8_CH1 (AF5_TIM8) by
 * HAL_TIM_MspPostInit(&htim8) at the end of this function. */
static void MX_TIM8_Init(void)
{
    TIM_ClockConfigTypeDef         sClockSourceConfig  = {0};
    TIM_MasterConfigTypeDef        sMasterConfig        = {0};
    ...
    /* Configures PB6 → TIM8_CH1 (AF5_TIM8) via stm32g4xx_hal_msp.c */
    HAL_TIM_MspPostInit(&htim8);
}

/* TIM15 — right motor PWM. PB15 is configured as TIM15_CH2 (AF1_TIM15 on
 * STM32G4) by HAL_TIM_MspPostInit(&htim15) at the end of this function. */
static void MX_TIM15_Init(void)
{
    TIM_ClockConfigTypeDef         sClockSourceConfig  = {0};
    TIM_MasterConfigTypeDef        sMasterConfig        = {0};
    ...
    /* Configures PB15 → TIM15_CH2 (AF1_TIM15 on STM32G4!) via msp.c */
    HAL_TIM_MspPostInit(&htim15);
}

/* =========================================================================
 * GPIO INITIALISATION  (only pins NOT owned by an MspInit live here)
 *
 *   Outputs:
 ...
 *
 *   PUL pins (PB6, PB15) are configured via HAL_TIM_MspPostInit() — not
 *   here. ADC, I2C, FDCAN GPIO are configured by their MspInit callbacks.
 * ========================================================================= */
static void MX_GPIO_Init(void)
{
