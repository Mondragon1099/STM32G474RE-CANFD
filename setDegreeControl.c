...
/* ===========================================================================
 * POSITION CONTROL — output-shaft-degree API
 * ----------------------------------------------------------------------------
 * GEAR_RATIO         motor revs per output rev (20:1 reduction here)
 * POS_PEAK_PPS       peak pulse rate during a goto. RPM_TO_PPS(400) gives
 *                    20 output RPM = 120 output°/sec — your test value.
 * POS_KP             P gain. Higher = sharper decel into target, more chance
 *                    of overshoot. Lower = lazier arrival, no overshoot.
 * POS_DEADBAND_STEPS error magnitude that counts as "arrived". 5 motor steps
 *                    = ~0.028° at the output.
 * =========================================================================== */
...
static float pos_target_steps  = 0.0f;    /* setpoint in MOTOR steps          */
static float pos_current_steps = 0.0f;    /* tracked from rate_out            */
...
/* ===========================================================================
 * POSITION CONTROL API — all non-blocking, all safe to call any time
 * ===========================================================================
 *
 *   stepper_set_target_deg(d)     set absolute target in OUTPUT degrees
 *   stepper_move_deg(d)           shift the target by d output degrees
 *   stepper_get_output_deg()      read current commanded output position
 *   stepper_at_target()           1 when within deadband, else 0
 *   stepper_position_tick()       run one control tick (every RATE_STEPPER_MS)
 *
 * The setpoint can be changed at any moment — including from inside an ISR
 * or another loop. Float writes aren't atomic on Cortex-M, so in the worst
 * case a tick will see a half-updated target and produce one slightly-off
 * command — harmless at your update rates.
 * =========================================================================== */
...
void stepper_position_tick(void)
{
    ...
    /* Integrate the actually-executed rate (post-ramp) into commanded
     * position. Using rate_out, not cmd_pps, keeps the tracker honest
     * during direction reversals and enable settle. */
